...
**Bug #4, encontrado pero NO corregido todavía al momento de este commit** (es código de NVIDIA en `pipeline/causal_diffusion_inference.py`, no algo de esta sesión): al inyectar el bloque de imagen de referencia (líneas ~387 y ~417), el código usa el `conditional_dict` **completo sin recortar** (con batch = número total de bloques del video, ej. 7) en vez de la versión correctamente recortada por bloque (`conditional_dict_list[block_index]`, batch=1). Esto rompe la atención cruzada cuando el número de bloques no divide exactamente el largo de secuencia por bloque (7040 tokens a resolución completa) — ej. con 7 bloques (56 frames) falla con `AssertionError: L1 (7040) must be divisible by num_chunks (7)`. Con 4 o 12 bloques "funciona" por coincidencia numérica (7040 es divisible entre 4 pero no entre 7 ni 12... ojo, verificar antes de asumir que "funciona" en otros conteos). **Fix propuesto, pendiente de aplicar**: cambiar `conditional_dict=conditional_dict` por `conditional_dict=conditional_dict_list[block_index]` (y el equivalente con índice 0 en la rama `independent_first_frame=True`) en las líneas 387 y 417.

Probado con éxito (antes de encontrar el bug #4, que solo aparece con ciertos conteos de bloques): imagen de referencia real (frame extraído de un video ya generado) → primer frame del resultado idéntico a la referencia, frames posteriores muestran movimiento coherente con el prompt.
...
1. Aplicar el fix del bug #4 (arriba) en `pipeline/causal_diffusion_inference.py`.
2. Probar la combinación 9:16 + imagen de referencia + modo Rápido/Turbo (LightVAE) — no probada todavía.
3. Si se quiere revisitar cuantización INT8 en el futuro, primero evaluar si vale la pena subir `torch` a >=2.11 para que `torchao` cargue sus kernels compilados (no intentado en esta sesión, riesgo de romper el setup actual).
