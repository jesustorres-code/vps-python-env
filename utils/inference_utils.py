...
def load_generator_checkpoint(generator, checkpoint_path: str, *, use_ema: bool = False, strict: bool | None = None):
    ...
    if strict is None:
        strict = not use_ema
    # generator's own weights (from CausalWanModel.from_pretrained, already
    # discarded the moment this checkpoint lands) are never read back, so
    # assign=True replaces parameters directly instead of copy_-ing into
    # them - skips materializing the from_pretrained placeholder entirely.
    return generator.load_state_dict(state_dict, strict=strict, assign=True)
